...
class TestElevatorAlgorithm(unittest.TestCase):
    @classmethod
    def setUpClass(self):
        #self.building = Building(simple_floor_populations, simple_dest_floors_by_state_name, simple_elevator_starting_floors, simple_elevator_capacity, simple_elevator_algorithm, simple_elevator_steps_per_load)
        return
    ...
    def test_assign_stop_SWS(self):
        ...
        building = Building(simple_floor_populations, simple_dest_floors_by_state_name, simple_elevator_algorithm, simple_elevator_starting_floors, simple_elevator_capacities, simple_elevator_steps_per_loads, simple_elevator_return_to_floors)
        ...
        building = Building(simple_floor_populations, simple_dest_floors_by_state_name, simple_elevator_algorithm, simple_elevator_starting_floors, simple_elevator_capacities, simple_elevator_steps_per_loads, simple_elevator_return_to_floors)
        building.elevators[0].is_active = True
        building.elevators[0].is_idle = False
        building.elevators[0].is_moving_up = True

        building.elevators[1].is_active = True
        building.elevators[1].is_idle = False
