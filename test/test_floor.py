...
class TestSchedule(unittest.TestCase):
    ...
    def setUpClass(self):
        ...
        for i in range(10): # Produces floors with ids 0-9 (10 floors)
            floor = Floor(id=i, num_residents=i, building_dest_floors_by_state_name=test_dest_floors_by_state_name)
    ...
    def test_unique_memory(self):
        ...
        for i in range(len(self.floors)):
            ...
            self.assertEqual(i, self.floors[i].num_residents)
            self.assertEqual(i, len(self.floors[i].people_on_floor))
            ...
            self.assertNotEqual(len(self.floors[i].people_going_up), len(self.floors[i].people_going_up))
            ...
            self.assertNotEqual(len(self.floors[i].people_going_down), len(self.floors[i].people_going_down))
            # Revert to original state for next iteration
            self.floors[i].people_on_floor.append(person)
