...
class TestSchedule(unittest.TestCase):
    ...
    def setUpClass(self):
        ...
        self.floors = []
        for i in range(1, 11): # Produces floors with ids 1-10
    ...
    def test_unique_memory(self):
        ...
        for i in range(len(self.floors)):
            self.assertEqual(i+1, self.floors[i].id)
            ...
            self.assertNotEqual(len(self.floors[i-1].people_going_up), len(self.floors[i].people_going_up))
            ...
            self.assertNotEqual(len(self.floors[i-1].people_going_down), len(self.floors[i].people_going_down))
            # Revert to original state for next iteration
            self.floors[i].people_on_floor.append(person)
